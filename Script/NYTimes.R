# environment ----
setwd(paste("C:/users/",Sys.getenv("USERNAME"),"/dropbox/rprojects/nytimes",sep=""))
...
train$NewsDesk<-factor(train$NewsDesk,
                          levels=c("Business","","Culture","Foreign",
                                   "Magazine","Metro","OpEd",
                                   "Science","Sports","Styles",
...
test$NewsDesk<- factor(test$NewsDesk,
                       levels=c("Business","","Culture","Foreign",
                                "Magazine","Metro","OpEd",
                                "Science","Sports","Styles",
                                "Travel","TStyle","National"))

test$SectionName<- factor(test$SectionName, 
                          levels=levels(train$SectionName))


test$SubsectionName<- factor(test$SubsectionName,
                             levels=levels(train$SubsectionName))

test$PubDate<- as.POSIXct(test$PubDate,format="%Y-%m-%e")
test$wkday<- factor(weekdays(test$PubDate),
                    levels=levels(train$wkday))
